...
https://datawrapper.dwcdn.net/6sabt/1/
...
Grouping the data by county name and listing the number of loans in each county
...
**Answer** Describe what the code below is doing and the output.
Removes any counties from the data that have less than 6 loans. The new data frame has 24 counties instead of 32.
...
A 5 digit number that is the combination of the state code and county code.
...
The md_counties table with the data from md_fips added to the right.
...
The map now only shows Maryland instead of the entire U.S.
...
**Answer** Describe the changes from the previous map
When you hover over a county the tootlitp now shows the county name and the number of loans is formatted with a comma.
...
https://datawrapper.dwcdn.net/e8ZWa/1/