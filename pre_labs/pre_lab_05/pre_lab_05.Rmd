...
**Answer**
There are several different spellings of the city Aberdeen.
...
**Task** Run the following code to make a copy of the city field called city_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called clean_md_loans.  Click the black arrow on the table to examine the city and city_clean columns.  Looking at the results on the first page only (first 10 rows), name two changes the refinr package made, by comparing city to city_clean.
**Answer**
Phoenix and Westminster have been changed from capitalized to title case.
...
**Answer**
Abell, Aberdeen, and Aberdeen Proving Ground have been changed from all caps to title case.
...
**Answer**
No, the changes to the first entries are exactly the same as key_collision_merge.
...
``` {r}
maryland_ppp_with_clean_cities <- read_csv("data/ppp_loans_md_with_clean_cities.csv.zip")
```
``` {r}
maryland_ppp_with_clean_cities %>% 
  group_by(city) %>% 
  summarise(
    count = n()
  ) %>% 
