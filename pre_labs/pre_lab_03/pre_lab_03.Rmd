...
```{r}
ppp_maryland_loans <- read_csv("ppp_loans_md.csv.zip", guess_max=1000)
...
**Answer**:
192,959 businesses are listed as NA. 275 loans were given to Subway franchises.
...
**Answer**:
135073. This matches the results from the previous code blocks.
...
**Answer**
There is no data for October of 2020
```{r}
ppp_maryland_loans %>%
...
**Answer**
The largest loan was for $10 million and the smallest was for $6. The largest loan is not suspicious because it represents the maximum amount a business could receive, but the $6 loan does seem odd since a $6 loan does not seem very helpful in keeping a business open.
```{r}
...
**Answer**
There are 2 loans for less than $100.
```{r}
...
**Answer**
All of the fields with loan amounts are consistent.
...
**Answer**
Improperly named headers, mismatched data types, and duplicates
...
**Answer**
It adds an 'x' in front so that the header no longer starts with a number
...
This error means that we are trying to perform a math operation on data that is of type string instead of number.
...
**Answer**
There are 2 duplicate entries for Stuart Jones LLC.
...
**Answer**
Takes a substring of the text field zip starting at the first letter and ending at the 5th letter.
...
**Task** Run the following codeblock to use str_to_title() to standardize capitalization in the "city" field. How many misspellings of Arnold remain after running this code? Answer below. Add a description of what this code does to your reference notebook.
**Answer**
There are still 2 misspellings remaining.
...
**Answer**
There is 1 misspelling remaining.
...
**Answer**
There are no more misspelling remaining.
