...
A4.
Smith is the owner of throwbackjerseys.com (which appears to be a business on paper only) and is accused of participating in a scheme to steal millions in PPP loans. I would assign a reporter to cover this story because the complaint outlines a conspiracy involving multiple people to defraud the PPP and how they were able to receive over 17 million dollars in loans. The amount of money and the conspiratorial nature of the crime make the story newsworthy.
...
wiki_tables <- url %>% 
  read_html() %>% 
  html_table()
courts_all <- wiki_tables[[3]]
courts_not_filed <- courts_all
for(i in 1:nrow(courts_not_filed)){
  for (j in 1:nrow(courts_filed)) {
    if(!is.na(courts_not_filed$Region[i]) & trimws(courts_not_filed$Region[i]) ==  trimws(courts_filed$region[j])){
      courts_not_filed <- courts_not_filed %>% slice(-i)
    }
  }
}

print(nrow(courts_filed)/nrow(courts_all) * 100)
```
...
A6.
As we saw before limitations on the available data can make it difficult to identify genuine fraud in the program. The case mentioned earlier relied on a cooperating witness for evidence. Without such evidence or a reccomendation from the bank issuing the loan most fraud went undetected.
...
main_table <- url %>% read_html() %>% 
  html_table()
state_info <- main_table[[1]]

all_states <- tibble()

for(row_number in 1:nrow(state_info)){
  state_table <- state_info$url[row_number] %>% 
    read_html() %>% 
    html_table()
...
all_states %>% 
  summarise(
    state,
    per_10k = total_ppp_loans / population * 10000
  ) %>% 
